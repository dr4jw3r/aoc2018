...
function init () {
    ...
    let sleptMinute = -1;
    let sleptMinuteValue = -1;
    let sleepingGuard = null;

    for (const guard of guards) {
        guard.getTotalSleep();
        const mostSleptMinute = guard.minutesSlept.indexOf(Math.max.apply(null, guard.minutesSlept));
        const value = guard.minutesSlept[mostSleptMinute];

        if (value > sleptMinuteValue) {
            sleptMinuteValue = value;
            sleepingGuard = guard;
            sleptMinute = mostSleptMinute;
        }
    }

    const answer = sleepingGuard.id * sleptMinute;
    console.log(answer);
}
